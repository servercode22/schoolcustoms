...
/* ======Amir Code======= */
	...
	public function checkSibling($id){
		$id=base64_decode(base64_decode($id));
		echo $this->student_model->siblingCount($id);
		
	}

	public function siblingInvoice($id){
		$id=base64_decode(base64_decode($id));
		$this->db->where('parent_id',$id);
		$result=$this->db->get('students')->result();
		$voucher= $this->voucher_settings_model->show();
		$data=[];		
			$guardianName=$result[0]->guardian_name;
			$guardianPhone=$result[0]->guardian_phone;
			$guardianRelation=$result[0]->guardian_relation;
			foreach($result as $item){
				$student=[];
				$sibling = $this->student_model->getByStudentSession($item->id);
				$student['name']="$sibling[firstname] $sibling[lastname]";
				

				$student_due_fee = $this->studentfeemaster_model->getStudentFees($item->id);

				$student_discount_fee = $this->feediscount_model->getStudentFeesDiscount($item->id);
				$total=0;
				foreach($student_due_fee as $key=>$item){
					
					foreach($item->fees as $k=>$fee){
						if($key===array_key_first($item->fees)){
				...
				foreach($student_discount_fee as $j => $dis){
					$total-=$dis['amount'];
				}
				$student['fee']=$total;
				array_push($data,$student);
			}
			$this->load->view('invoice/siblingInvoice',['data'=>$data,'voucher'=>$voucher,'guardianName'=>$guardianName,'guardianRelation'=>$guardianRelation,'guardianPhone'=>$guardianPhone,'dueDate'=>$dueDate]);
	}
/* ========End Amir Edit======= */
